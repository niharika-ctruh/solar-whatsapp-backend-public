...
const sendRequest = (body, user) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield hosts.updateMany({ _id: { $in: hostIds } }, {
            $push: {
                visitRequests: {
        ...
        const updatedCustomer = yield customer_model_1.Customer.findByIdAndUpdate(newCustomer._id, { requestId: savedRequest._id }, { new: true });
        return {
...
const getVisitRequestCounts = (filters) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const [globalData, filteredData] = yield Promise.all([globalCountsPromise, filteredCountsPromise]);
...
const getVisitRequests = (filters, skip, limit) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const match = buildBaseMatch(filters);
    const basePipeline = buildFullPipeline(match, filters);
    const countResult = yield visitRequest_model_1.VisitRequest.aggregate([...basePipeline, { $count: "total" }]);
    ...
    const requests = yield visitRequest_model_1.VisitRequest.aggregate(listPipeline);
    return { requests, total };
});
exports.getVisitRequests = getVisitRequests;
const getVisitRequestById = (_a) => __awaiter(void 0, [_a], void 0, function* ({ id }) {
    const pipeline = [
        {
    ...
    const result = yield visitRequest_model_1.VisitRequest.aggregate(pipeline);
    if (!result.length)
        throw new helper_error_1.Errors("Request not found", 404);
    return result[0];
});
exports.getVisitRequestById = getVisitRequestById;
const updateVisitRequestById = (_a) => __awaiter(void 0, [_a], void 0, function* ({ id, body, user, }) {
    const existingRequest = yield visitRequest_model_1.VisitRequest.findById(id);
    if (!existingRequest)
        throw new helper_error_1.Errors("Request not found", 404);
    ...
    const updated = yield visitRequest_model_1.VisitRequest.findByIdAndUpdate(id, updatedRequest, { new: true });
    return updated;
});
exports.updateVisitRequestById = updateVisitRequestById;
//# sourceMappingURL=sendRequest.service.js.map